...
local function downloadFile(path, func)
	if not isfile(path) then
		...
		if path:find('.lua') then
			res = '--此水印用于在文件缓存时删除文件，移除它可使文件在vape更新后保留。\n'..res
		end
		writefile(path, res)
	end
...
local function serverHop(pointer, filter)
	...
	if data and data.data then
		...
		if data.nextPageCursor then
			...
			notif('Vape', '未能找到可用的服务器。', 5, 'warning')
		end
	else
			...
			if exp and exp:WaitForChild('appLayout', 5) then
				vape:Clean(exp:FindFirstChild('RCTScrollContentView', true).ChildAdded:Connect(function(obj)
					...
					if obj and obj:IsA('TextLabel') then
						...
						if obj.ContentText:sub(1, 35) == '你现在正在与' then
							obj.Visible = false
						end
					end
				end))
			...
			if whitelist.textdata ~= whitelist.olddata then
				if whitelist.data.Announcement.expiretime > os.time() then
					...
					if table.find(targets, tostring(lplr.UserId)) then
						...
						hint.Text = 'VAPE公告: '..whitelist.data.Announcement.text
...
run(function()
	...
	Mode = Reach:CreateDropdown({
		...
		List = {'触碰事件', '调整大小'},
		Function = function(val)
			Chance.Object.Visible = val == '触碰事件'
		end,
		Tooltip = '触碰事件 - 向服务器报告虚假的碰撞事件\n调整大小 - 物理上修改工具的大小'
	})
...
run(function()
	...
	Method = SilentAim:CreateDropdown({
		...
		Tooltip = 'FindPartOnRay* - 旧版游戏中使用的已弃用的射线投射方法\nRaycast - 现代射线投射方法\nPointToRay - 从屏幕坐标生成射线的方法\nRay - 挂钩 Ray.new'
	})
	...
	SilentAim:CreateToggle({
		...
		Function = function(callback)
			if callback then
				...
				CircleObject.NumSides = 100
				CircleObject.Transparency = 1 - CircleTransparency.Value
				CircleObject.Visible = SilentAim.Enabled and Mode.Value == '鼠标'
			else
				pcall(function()
					CircleObject.Visible = false
					CircleObject:Remove()
...
run(function()
	...
	Method = AntiFall:CreateDropdown({
		...
		Function = function(val)
			if Mode.Object then
				Mode.Object.Visible = val == '部件'
				Material.Object.Visible = val == '部件'
				Color.Object.Visible = val == '部件'
			end
		...
		Tooltip = '部件 - 在你下方移动一个部件，通过各种方法阻止你掉落\n经典 - 到达部件销毁平面后将你传送出虚空'
	})
...
run(function()
	...
	Lunge = Killaura:CreateToggle({Name = '仅限剑冲刺'})
...
run(function()
	...
	MouseTP = vape.Categories.Blatant:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				if MovementMode.Value ~= '平滑移动' then
					MouseTP:Toggle()
					if entitylib.isAlive then
						if MovementMode.Value == '马达' then
	...
	MovementMode = MouseTP:CreateDropdown({
		...
		List = {'坐标系', '马达', '平滑移动'},
		Function = function(val)
			Length.Object.Visible = val == '平滑移动'
			Delay.Object.Visible = val == '平滑移动'
		end
	})
...
run(function()
	...
	Phase = vape.Categories.Blatant:CreateModule({
		...
		Tooltip = '让你穿过墙壁。(按住Shift键可在蜘蛛功能之上使用穿墙)'
	})
	Mode = Phase:CreateDropdown({
		Name = '模式',
		List = {'部件', '角色', '坐标系', '马达', '标志'},
		Function = function(val)
			StudLimit.Object.Visible = val == '坐标系' or val == '马达'
			if fflag then
				setfflag('AssemblyExtentsExpansionStudHundredth', '30')
			end
			for part in modified do
			...
			if callback then
				Speed:Clean(runService.PreSimulation:Connect(function(dt)
					if entitylib.isAlive and not Fly.Enabled and not LongJump.Enabled then
						...
						local movevec = TargetStrafeVector or Options.MoveMethod.Value == '直接' and calculateMoveVector(Vector3.new(a + d, 0, w + s)) or entitylib.character.Humanoid.MoveDirection
...
run(function()
	...
	Spider = vape.Categories.Blatant:CreateModule({
		...
		Tooltip = '让你爬上墙壁。(按住Shift键可在蜘蛛功能之上使用穿墙)'
	})
	Mode = Spider:CreateDropdown({
		Name = '模式',
		List = {'速度', '冲量', '坐标系', '部件'},
		Function = function(val)
			Value.Object.Visible = val ~= '部件'
			State.Object.Visible = val ~= '部件'
			if Truss then
				Truss:Destroy()
				Truss = nil
			end
...
run(function()
	...
	Mode = SpinBot:CreateDropdown({
		...
		Function = function(val)
			...
			AngularVelocity = val == '身体动力' and Instance.new('BodyAngularVelocity') or nil
		end
	})
...
run(function()
	...
	Mode = Chams:CreateDropdown({
		...
		List = {'高亮', '方框手柄'},
		Function = function(val)
			OutlineColor.Object.Visible = val == '高亮'
			OutlineTransparency.Object.Visible = val == '高亮'
			if Chams.Enabled then
				Chams:Toggle()
				Chams:Toggle()
			end
...
run(function()
	...
	for _, name in {'X轴旋转', 'Y轴旋转', 'Z轴旋转'} do 
...
run(function()
	...
	vape.Libraries.sessioninfo:AddItem('游戏时长', os.clock(), function(value)
...
run(function()
	...
	local function playAnimation(char)
		...
		if suc then
			...
			notif('动画播放器', '加载动画失败 : '..(res or '无效的动画ID'), 5, '警告')
		end
	end
...
run(function()
	...
	AutoRejoin = vape.Categories.Utility:CreateModule({
		...
		Tooltip = '当你被踢出或断开连接时自动加入新服务器'
	})
	Sort = AutoRejoin:CreateDropdown({
		Name = '排序方式',
		List = {'降序', '升序'},
		Tooltip = '降序 - 优先选择人多的服务器\n升序 - 优先选择人少的服务器'
	})
end)
...
run(function()
	...
	ChatSpammer = vape.Categories.Utility:CreateModule({
		...
		Function = function(callback)
			if callback then
				...
				local ind = 1
				repeat
					local message = (#Lines.ListEnabled > 0 and Lines.ListEnabled[math.random(1, #Lines.ListEnabled)] or 'WTB-Vape天下第一')
	...
	Lines = ChatSpammer:CreateTextList({Name = '消息列表'})
...
run(function()
	...
	ServerHop:CreateButton({
		Name = '重新加入上一个服务器',
		Function = function()
			notif('服务器跳转', shared.vapeserverhopprevious and '正在重新加入上一个服务器...' or '找不到上一个服务器', 5)
...
run(function()
	...
	local function playerAdded(plr)
		...
		if user or getRole(plr, tonumber(Group.Value) or 0) >= (tonumber(Role.Value) or 1) then
			notif('管理员检测', '检测到管理员 ('..(user and '黑名单用户' or '管理员权限')..'): '..plr.Name, 60, 'alert')
	...
	StaffDetector = vape.Categories.Utility:CreateModule({
		...
		Tooltip = '检测游戏中拥有管理员权限的玩家'
	})
Mode = StaffDetector:CreateDropdown({
		Name = '模式',
		List = {'卸载注入', '服务器跳转', '配置档案', '自动配置', '通知'},
		Function = function(val)
			if Profile.Object then
				Profile.Object.Visible = val == '配置档案'
			end
		end
	})
		...
		Function = function(callback)
			if callback then
				...
				if module and module.activeCameraController and Freecam.Enabled then
					...
					contextService:BindActionAtPriority('自由视角键盘控制'..randomkey, function()
				...
				pcall(function()
					contextService:UnbindAction('自由视角键盘控制'..randomkey)
				end)
				if module and old then
